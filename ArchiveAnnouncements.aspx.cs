...
public partial class Admin_ArchiveAnnouncements : System.Web.UI.Page
{
    SqlConnection con;
    SqlCommand cmd;
    SqlDataReader dr;
    ...
    private string bindData(string dataBind = "")
    {
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (adr = new SqlDataAdapter("SelectTotalArchivedAnnouncements", con))
            {
                dt = new DataTable();
                adr.Fill(dt);
                GridView1.DataBind();
    ...
    private void deleteRecord(string announcement_ID)
    {
        ...
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (cmd = new SqlCommand("addTrashArchiveAnnouncements",con))
            {
            ...
            using (cmd = new SqlCommand("trashArchiveAnnouncements", con))
            {
                cmd.Parameters.AddWithValue("announcementID", announcement_ID);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                lblArchiveAnnouncementCount.Visible = true;

                using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
                    con.Open();
                {
                    using (cmd = new SqlCommand("CountTotalArchivedAnnouncements", con))
                    {
                        ...
                        if (dr.HasRows)
                        {
                            while (dr.Read())
                            {
                                lblArchiveAnnouncementCount.Text = dr["totalPostedArchives"].ToString();
                            }
                        }
                        dr.Close();
                        cmd.Dispose();
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        {
            SqlDataSource1.SelectCommand = "SELECT * FROM Archived_Announcements WHERE (announcement_subject LIKE '" + searchPost + "') OR (announcement LIKE '" + searchPost + "') OR (sender_name LIKE '" + searchPost + "') OR (date_posted LIKE '" + searchPost + "')";
        }
    }
    ...
    protected void btnExcelExport_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("content-disposition", "attachment;filename=Announcement_Archives.xls");
        Response.Charset = "";
