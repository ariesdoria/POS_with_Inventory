...
public partial class Admin_ArchiveReturnedItems : System.Web.UI.Page
{
    SqlConnection con;
    SqlCommand cmd;
    SqlDataReader dr;
    ...
    private string bindData(string dataBind = "")
    {
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (adr = new SqlDataAdapter("SelectTotalArchivedReturnItems", con))
            {
                dt = new DataTable();
                adr.Fill(dt);
                GridView1.DataBind();
    ...
    private void deleteRecord(string return_ID)
    {
        ...
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (cmd = new SqlCommand("addTrashArchiveReturnedItems", con))
            {
                cmd.Parameters.AddWithValue("returnID", return_ID);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
            }
            ...
            using (cmd = new SqlCommand("trashArchiveReturnItems", con))
            {
                cmd.Parameters.AddWithValue("returnID", return_ID);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                lblArchiveReturnedItemCount.Visible = true;

                using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
                    con.Open();
                {
                    using (cmd = new SqlCommand("CountTotalArchivedReturnedItems", con))
                    {
                        ...
                        if (dr.HasRows)
                        {
                            while (dr.Read())
                            {
                                lblArchiveReturnedItemCount.Text = dr["totalReturns"].ToString();
                            }
                        }
                        dr.Close();
                        cmd.Dispose();
    ...
    protected void btnSearchReturnedItem_Click(object sender, EventArgs e)
    {
        string errorMessage = "<script language='javascript'>alert('Search field is empty!')</script>";
        string searchReturnItem;

        searchReturnItem = Convert.ToString(txtSearchReturnedItem.Text);

        if (txtSearchReturnedItem.Text == "")
        {
        ...
        {
            SqlDataSource1.SelectCommand = "SELECT * FROM Archived_Item_Return WHERE (ItemName LIKE '" + searchReturnItem + "') OR (Category LIKE '" + searchReturnItem + "') OR (Description LIKE '" + searchReturnItem + "') OR (CostPrice LIKE '" + searchReturnItem + "') OR (SellingPrice LIKE '" + searchReturnItem + "') OR (Quantity LIKE '" + searchReturnItem + "') OR (Condition LIKE '" + searchReturnItem + "') OR (DateReturned LIKE '" + searchReturnItem + "')";
        }
    }

    protected void btnClearSearch_Click(object sender, EventArgs e)
    {
        txtSearchReturnedItem.Text = "";
    }
    ...
    protected void btnExcelExport_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("content-disposition", "attachment;filename=Return_Item_Archives.xls");
        Response.Charset = "";
