...
public partial class Admin_ArchiveStocks : System.Web.UI.Page
{
    SqlConnection con;
    SqlCommand cmd;
    SqlDataReader dr;
    ...
    private string bindData(string dataBind = "")
    {
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (adr = new SqlDataAdapter("SelectTotalArchivedStocks", con))
            {
                dt = new DataTable();
                adr.Fill(dt);
                GridView1.DataBind();
    ...
    private void deleteRecord(string item_ID)
    {
        ...
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (cmd = new SqlCommand("addTrashArchiveStocks", con))
            {
                cmd.Parameters.AddWithValue("stockID", item_ID);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
            }
            ...
            using (cmd = new SqlCommand("trashArchiveStocks", con))
            {
                cmd.Parameters.AddWithValue("productID", item_ID);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
            }
            con.Close();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                lblArchiveItemCount.Visible = true;

                using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
                    con.Open();
                {
                    using (cmd = new SqlCommand("CountTotalArchivedStocks", con))
                    {
                        ...
                        if (dr.HasRows)
                        {
                            while (dr.Read())
                            {
                                lblArchiveItemCount.Text = dr["totalStocks"].ToString();
                            }
                        }
                        dr.Close();
                        cmd.Dispose();
    ...
    protected void btnSearchItem_Click(object sender, EventArgs e)
    {
        ...
        {
            SqlDataSource1.SelectCommand = "SELECT * FROM Archived_Stocks WHERE (ItemName LIKE '" + searchItem + "') OR (Category LIKE '" + searchItem + "') OR (Description LIKE '" + searchItem + "') OR (CostPrice LIKE '" + searchItem + "') OR (SellingPrice LIKE '" + searchItem + "') OR (Quantity LIKE '" + searchItem + "') OR (RemainingStock LIKE '" + searchItem + "') OR (DatePurchased LIKE '" + searchItem + "')";
        }
    }
    ...
    protected void btnExcelExport_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("content-disposition", "attachment;filename=Item_Archives.xls");
        Response.Charset = "";
