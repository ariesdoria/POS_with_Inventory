...
public partial class Admin_ArchiveEmployee : Page
{
    SqlConnection con;
    SqlCommand cmd;
    SqlDataReader dr;
    ...
    private string bindData(string dataBind = "")
    {
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (adr = new SqlDataAdapter("SelectTotalArchivedEmployee", con))
            {
                dt = new DataTable();
                adr.Fill(dt);
                GridView1.DataBind();
    ...
    private void deleteRecord(string account_ID)
    {
        ...
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (cmd = new SqlCommand("addTrashEmployee", con))
            {
                cmd.Parameters.AddWithValue("accountNumber", account_ID);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
            }
            using (cmd = new SqlCommand("updateTrashEmployee", con))
            {
                cmd.Parameters.AddWithValue("trashStatus", status);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
            }
            using (cmd = new SqlCommand("deleteArchivedEmployee", con))
            {
                cmd.Parameters.AddWithValue("employeeID", account_ID);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
            }
            con.Close();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                lblArchiveEmployeeCount.Visible = true;

                using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
                    con.Open();
                {
                    using (cmd = new SqlCommand("CountTotalArchivedEmployees", con))
                    {
                        ...
                        if (dr.HasRows)
                        {
                            while (dr.Read())
                            {
                                lblArchiveEmployeeCount.Text = dr["totalEmployee"].ToString();
                            }
                        }
                        dr.Close();
                        cmd.Dispose();
    ...
    protected void btnSearchEmployee_Click(object sender, EventArgs e)
    {
        ...
        {
            SqlDataSource1.SelectCommand = "SELECT * FROM Archived_Employee WHERE (empAccountNumber LIKE '" + searchEmployee + "') OR (empFirstname LIKE '" + searchEmployee + "') OR (empLastname LIKE '" + searchEmployee + "') OR (empGender LIKE '" + searchEmployee + "') OR (empPhone LIKE '" + searchEmployee + "') OR (empLocation LIKE '" + searchEmployee + "') OR (date_hired LIKE '" + searchEmployee + "')";
        }
    }
    ...
    protected void btnExcelExport_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("content-disposition", "attachment;filename=Employee_Archives.xls");
        Response.Charset = "";
