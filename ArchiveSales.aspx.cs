...
public partial class Admin_ArchiveSales : System.Web.UI.Page
{
    SqlConnection con;
    SqlCommand cmd;
    SqlDataReader dr;
    ...
    private string bindData(string dataBind = "")
    {
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (adr = new SqlDataAdapter("SelectTotalArchivedSales", con))
            {
                dt = new DataTable();
                adr.Fill(dt);
                GridView1.DataBind();
    ...
    private void deleteRecord(string sales_ID)
    {
        ...
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (cmd = new SqlCommand("addTrashArchiveSales", con))
            {
            ...
            using (cmd = new SqlCommand("trashArchiveSales", con))
            {
                cmd.Parameters.AddWithValue("salesID", sales_ID);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                lblArchiveSalesCount.Visible = true;

                using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
                    con.Open();
                {
                    using (cmd = new SqlCommand("CountTotalArchivedSales", con))
                    {
                        ...
                        if (dr.HasRows)
                        {
                            while (dr.Read())
                            {
                                lblArchiveSalesCount.Text = dr["totalSales"].ToString();
                            }
                        }
                        dr.Close();
                        cmd.Dispose();
    ...
    protected void btnSearchSales_Click(object sender, EventArgs e)
    {
        ...
        {
            SqlDataSource1.SelectCommand = "SELECT * FROM Archived_Sales WHERE (ItemName LIKE '" + searchSales + "') OR (Category LIKE '" + searchSales + "') OR (Description LIKE '" + searchSales + "') OR (CostPrice LIKE '" + searchSales + "') OR (SellingPrice LIKE '" + searchSales + "') OR (DateSold LIKE '" + searchSales + "') OR (Quantity_Sold LIKE '" + searchSales + "')";
        }
    }
    ...
    protected void btnExcelExport_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("content-disposition", "attachment;filename=Sales_Archives.xls");
        Response.Charset = "";
