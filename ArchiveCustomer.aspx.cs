...
public partial class Admin_ArchiveCustomer : System.Web.UI.Page
{
    SqlConnection con;
    SqlCommand cmd;
    SqlDataReader dr;
    ...
    private string bindData(string dataBind = "")
    {
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (adr = new SqlDataAdapter("SelectTotalArchivedCustomers", con))
            {
                dt = new DataTable();
                adr.Fill(dt);
                GridView1.DataBind();
    ...
    private void deleteRecord(string customer_ID)
    {
        ...
        using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
            con.Open();
        {
            using (cmd = new SqlCommand("addTrashCustomer", con))
            {
                cmd.Parameters.AddWithValue("customerID", customer_ID);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
            }
            using (cmd = new SqlCommand("updateTrashCustomer", con))
            {
                cmd.Parameters.AddWithValue("trashStatus", status);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
            }
            using (cmd = new SqlCommand("deleteArchivedCustomer", con))
            {
                cmd.Parameters.AddWithValue("customerID", customer_ID);
                cmd.ExecuteNonQuery();
                cmd.Dispose();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                lblArchiveCustomerCount.Visible = true;

                using (con = new SqlConnection(@"Data Source = ARIES - PC\SQLEXPRESS; AttachDbFilename = D:\Visual Studio 2015 Web Forms\POS_with_Inventory\Database\Point_Of_Sales.mdf;Integrated Security = True"))
                    con.Open();
                {
                    using (cmd = new SqlCommand("CountTotalArchivedCustomers", con))
                    {
                        ...
                        if (dr.HasRows)
                        {
                            while (dr.Read())
                            {
                                lblArchiveCustomerCount.Text = dr["ArchivedCustomers"].ToString();
                            }
                        }
                        dr.Close();
                        cmd.Dispose();
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        searchCustomer = Convert.ToString(txtSearchCustomer.Text);
        ...
        {
            SqlDataSource1.SelectCommand = "SELECT * FROM Archived_Customer WHERE (Firstname LIKE '" + searchCustomer + "') OR (Lastname LIKE '" + searchCustomer + "') OR (Email LIKE '" + searchCustomer + "') OR (PhoneNumber LIKE '" + searchCustomer + "') OR (Address LIKE '" + searchCustomer + "') OR (City LIKE '" + searchCustomer + "') OR (State LIKE '" + searchCustomer + "') OR (Zip LIKE '" + searchCustomer + "') OR (Country LIKE '" + searchCustomer + "') OR (CompanyName LIKE '" + searchCustomer + "')";
        }
    }
    ...
    protected void btnExcelExport_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("content-disposition", "attachment;filename=Customer_Archives.xls");
        Response.Charset = "";
